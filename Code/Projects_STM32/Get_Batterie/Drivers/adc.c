...
#define Buffer_size_One ((uint8_t) 1)
...
/**
* @fn Init_ADC_One_Channel
* @brief Initializes and strat the ADC converter 
* @param GPIO_TypeDef*, GPIO_pin number: one GPIO pin that used for ADC
* @param ADC_TypeDef*, ADC_Channel number, ADC_SampleTime£ºdefine an ADC among ADC1, ADC2, and ADC3 that will be used later; the channel of ADC associated and its sample time
* @return void
*/
void Init_ADC_One_Channel (GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin1, ADC_TypeDef* ADCx, uint8_t channel1, uint8_t ADC_SampleTime)
{
    //structure declaration  
    ...
    //Enable Rcc clock for ADC interface
	  Port_GPIO_Clock_Enable(GPIOx);
    ADC_Clock_Enable(ADCx);
	  RCC_ADCCLKConfig(RCC_PCLK2_Div6);
	
	//GPIO structure
	  GPIO_InitStruct.GPIO_Pin = GPIO_Pin1;// Set gpio pin for adc
    ...
    ADC_InitStruct.ADC_Mode = ADC_Mode_Independent ;// ADC1 and ADC3 are independent
		ADC_InitStruct.ADC_ScanConvMode = DISABLE ;//ADC works in single mode
	  ADC_InitStruct.ADC_ContinuousConvMode = ENABLE; ////Conversion is in continuous mode
    ADC_InitStruct.ADC_ExternalTrigConv = ADC_ExternalTrigConv_None ;
		ADC_InitStruct.ADC_DataAlign = ADC_DataAlign_Right ;// Aligne in Right
    ADC_InitStruct.ADC_NbrOfChannel = NbrOfChannel_One;// Channel of ADC
...
/**
* @fn ADC_Single_Channel_Value
* @brief get the ADC value of one channel
* @param void
* @return uint16_t; digital value of converted value
*/
uint16_t ADC_Single_Channel_Value(ADC_TypeDef* ADCx){
		//Start ADC Software Conversion
		ADC_SoftwareStartConvCmd(ADCx, ENABLE);
