...
#define PWM_PERIOD_US 3.0e3
#define PWM_FREQUENCY US_PER_S/PWM_PERIOD_US
#define TRIGGER_DC 0.01
#define CAPTURE_PRIORITY 3
#define AUXILIARY_VAR (0.5/0.137)
#define ULTRASSONIC_PARAMETER (58.2*(6.0/16.0)*(6/6.5))
#define ULTRASONIC_SATURATION 50.0
...
void Ultrasonic_Init_ITHandling(void){
   ...
   if (capture_IT_aux_var == 0){
         ...
         if( capture_val_falling_edge >= capture_val_rising_edge)
            period_us = ( (uint8_t) (capture_val_falling_edge - capture_val_rising_edge)*AUXILIARY_VAR*PWM_PERIOD_US)/65535.0;
         else
            period_us = ( (uint8_t) ((0xFFFF + capture_val_falling_edge + 1) - capture_val_rising_edge)*AUXILIARY_VAR*PWM_PERIOD_US)/65535.0;
...
void Ultrasonic_Start(void) {
   Capture_ITEnable(TIMx, CH_ECHO, CAPTURE_PRIORITY, Ultrasonic_Init_ITHandling);
   Capture_Enable(TIMx);
}
