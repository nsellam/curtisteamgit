...
void pwm_init(TIM_TypeDef *timer, uint16_t channel, float frequency_PWM_Hz){  
  ...
  if(channel == TIM_Channel_1)       TIM_OC1Init(timer, &TIM_OCInitStructure); 
  else if(channel == TIM_Channel_2)  TIM_OC2Init(timer, &TIM_OCInitStructure);
  else if(channel == TIM_Channel_3)  TIM_OC3Init(timer, &TIM_OCInitStructure);
  else if(channel == TIM_Channel_4)  TIM_OC4Init(timer, &TIM_OCInitStructure);
